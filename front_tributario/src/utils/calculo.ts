class SimplesNacionalAliquota {
  ...
  funcCalculaAliqEfetiva(rbt12: number, anexo: string): number | null {
    let alqEfetiva: number | null;
    if (anexo == 'Anexo I') {
      this._faixa = [
        [180000, 3 / 100, 0],
        [360000, 6.3 / 100, 5939],
    ...
    for (let i = 0; i < this._faixa.length; i++) {
      if (rbt12 < this._faixa[i][0]) {
        this.alq = this._faixa[i][1];
        this.pd = this._faixa[i][2];
        alqEfetiva = (((rbt12 * this.alq)) - this.pd) / rbt12;
  ...
  funcCalculaImpostoDetalhado(faturamento: number, anexo: string, alqEfetiva: number): {[key: string]: number} | null {
    ...
    for (let i = 0; i < this._faixa.length; i++) {
      if (this.rbt12 < this._faixa[i][0]) {
        impostos["IR"] = faturamento * ((alqEfetiva/100) * this._faixa[i][1]);
        impostos["CSLL"] = faturamento * ((alqEfetiva/100) * this._faixa[i][2]);
        impostos["PIS"] = faturamento * ((alqEfetiva/100) * this._faixa[i][3]);
        impostos["COFINS"] = faturamento * ((alqEfetiva/100) * this._faixa[i][4]);
        impostos["CPP"] = faturamento * ((alqEfetiva/100) * this._faixa[i][5]);
        impostos["ICMS"] = faturamento * ((alqEfetiva/100) * this._faixa[i][6]);
        impostos["ISS"] = faturamento * ((alqEfetiva/100) * this._faixa[i][7]);
        impostos["IPI"] = faturamento * ((alqEfetiva/100) * this._faixa[i][8]);
        return impostos;
      }
    }
    return null;
