...
export default function CardImposto({name,percent,value}:ICardImposto) {
  return (
        <Box color="white" border="3px solid gray" p={2} borderRadius="8px" position="relative" _hover={{border:"3px solid white"}} role='group'  >
            <Text 
            position="absolute" 
            top={-4} 
            bgColor="hermesBlue.400" 
            color="white" border="3px solid gray" _groupHover={{border:"3px solid white"}} borderRadius="8px" px={2} fontWeight="500" fontSize="18px"
            >{name}</Text>
